...
def measure_time(size):
    ...
    matmul_dense_np_time = timeit.timeit(lambda: dense_A @ dense_B, number=1)
    ...
    matmul_sparse_np_time = timeit.timeit(lambda: sparse_A @ sparse_B, number=1)
...
plt.plot(sizes, np.log(matmul_dense_np_times), label='MatMul Dense NP', marker='o')
...
plt.plot(sizes, np.log(matmul_sparse_np_times), label='MatMul Sparse NP', marker='o')
